...
const API_HOST = 'http://52.79.159.149:8080/api';
...
const PORTONE_COMPLETE_PAYMENT_URL = `${API_HOST}`;
...
// order 에서 호출되는 base url
const axiosPortOneInstance = (url, params, options) => {
  const instance = axios.create({
    baseURL: url,
    // headers: { Authorization: `Bearer ${token}` },
    params: params,
...
export const portOneApi = axiosPortOneInstance(PORTONE_COMPLETE_PAYMENT_URL);
...
/**
 * 일반적으로 호출되는 base url : 52.79.159.149
 *
 * @param {*} url / 뒤에 붙일 url
 * @param {*} params 전송할 데이터
 * @param {*} options
