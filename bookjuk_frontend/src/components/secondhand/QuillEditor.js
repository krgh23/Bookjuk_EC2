      ...
      if (file) {
        ...
        try {
          ...
          if (type === 'product') { // 상품에서 호출할때
            response = await axios.post('http://52.79.159.149:8080/admin/products/quill/upload', formData, {
              headers: {
                'Content-Type': 'multipart/form-data',
              },
            });
          } else { // 중고상품에서 호출할때
            response = await axios.post('http://52.79.159.149:8080/quill/upload', formData, {
              headers: {
                'Content-Type': 'multipart/form-data',
              },
            });
          ...
          quill.insertEmbed(range.index, 'image', 'http://52.79.159.149:8080/image/' + imageUrl); // 이미지 삽입

          // // 이미지 삽입 후 크기 조정
          // const imageElement = quill.root.querySelector(`img[src='http://52.79.159.149:8080/image/${imageUrl}']`);
          // if (imageElement) {
