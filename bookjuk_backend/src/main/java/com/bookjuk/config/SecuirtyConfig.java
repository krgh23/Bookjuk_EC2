...
public class SecuirtyConfig {
  ...
  CorsConfigurationSource corsConfigurationSource() {
    ...
    configuration.setAllowedOrigins(Arrays.asList("http://52.79.159.149")); // 리액트 서버
  ...
  SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
    ...
    httpSecurity.formLogin(login -> {
      login.loginPage("/api/user/login")             // GET "/user/login" 요청 시로그인 페이지로 이동함
           ...
           .defaultSuccessUrl("/api/user/loginSuccess", true);       // 로그인 성공시 UserController에 있는 @GetMapping("/loginSuccess") 확인 가능합니다.       
    });
